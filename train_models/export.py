...
TensorFlow.js:
    ...
    $ npm start
"""
##python export.py --weights weight/best.pt --include torchscript
...
def export_torchscript(model, im, file, optimize, prefix=colorstr('TorchScript:')):
    ...
    try:
        ...
        fl = file.with_suffix('.torchscript.ptl')                                                                             #转换模型为ptl
        ...
        LOGGER.info(f'{prefix} export success, saved as {fl} ({file_size(fl):.1f} MB)')                                  
        return fl
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/shrimp.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'weight/824model.pt', help='model.pt path(s)')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[2016,2016], help='image (h, w)')
    ...
    parser.add_argument('--iou-thres', type=float, default=0.30, help='TF.js NMS: IoU threshold')
    parser.add_argument('--conf-thres', type=float, default=0.40, help='TF.js NMS: confidence threshold')
